...
export const PENALTY_VISUAL_ASSETS = Object.freeze({
  ...
  singleAngle: Object.freeze({
    selectionBackground: "./assets/penalty-single-angle/broadcast-selection-0.9h4c.png",
    background: "./assets/penalty-single-angle/broadcast-action-0.9h4c.png",
...
// 0.9H4C calibrates interaction and replay to the uploaded broadcast frame.
// All six choices sit inside the visible goal around the goalkeeper.
export const BROADCAST_SELECTION_POINTS = Object.freeze({
  ball: Object.freeze({ x: 938, y: 530 }),
  keeper: Object.freeze({ x: 303, y: 392 }),
  zones: Object.freeze({
    "top-left": Object.freeze({ x: 88, y: 320 }),
    "top-centre": Object.freeze({ x: 232, y: 308 }),
    "top-right": Object.freeze({ x: 386, y: 322 }),
    "bottom-left": Object.freeze({ x: 82, y: 458 }),
    "bottom-centre": Object.freeze({ x: 226, y: 474 }),
    "bottom-right": Object.freeze({ x: 390, y: 460 })
  })
});
...
export const SINGLE_ANGLE_KEEPER_MOVES = Object.freeze({
  "top-left": Object.freeze({ asset: "keeperHighLeft", x: 38, y: 278, width: 270, height: 132, rotation: -0.035, label: "HIGH LEFT" }),
  "top-centre": Object.freeze({ asset: "keeperReady", x: 178, y: 268, width: 112, height: 166, rotation: 0, label: "HIGH CENTRE" }),
  "top-right": Object.freeze({ asset: "keeperHighRight", x: 188, y: 280, width: 270, height: 132, rotation: 0.035, label: "HIGH RIGHT" }),
  "bottom-left": Object.freeze({ asset: "keeperLowLeft", x: 44, y: 384, width: 250, height: 116, rotation: -0.020, label: "LOW LEFT" }),
  "bottom-centre": Object.freeze({ asset: "keeperReady", x: 204, y: 366, width: 104, height: 132, rotation: 0, label: "LOW CENTRE", scaleX: 1.08, scaleY: 0.88 }),
  "bottom-right": Object.freeze({ asset: "keeperLowRight", x: 194, y: 384, width: 250, height: 116, rotation: 0.020, label: "LOW RIGHT" })
});
...
export function singleAngleBallState(replay = {}, progress = 0) {
  ...
  return {
    ...
    visible: p >= strike - 0.004,
    shadow: { x: lerp(start.x, target.x, flightT), y: shadowY, radiusX: lerp(17, 5, flightT), radiusY: lerp(5, 2, flightT), opacity: clamp(0.20 - Math.max(0, shadowY - y) / 900, 0.025, 0.18) }
  };
}
...
export class PenaltyVisualPack {
  ...
  drawSingleAngleCinematic(ctx, replay, time = 0, reducedMotion = false, final = false) {
    ...
    const approachT = final ? 1 : smoothstep(clamp(progress / Math.max(0.001, strike), 0, 1));
    const followT = final ? 1 : smoothstep(clamp((progress - strike + 0.006) / 0.125, 0, 1));
    const contactAlpha = 1 - followT;
    if (contactAlpha > 0.01) {
      this.drawSingleAngleSprite(ctx, contactSprite, {
        x: lerp(1030, 770 + shotMove.translateX * 0.28, approachT),
        y: lerp(382, 326 + shotMove.translateY * 0.18, approachT),
        width: lerp(132, 180, approachT),
        height: lerp(164, 206, approachT),
        alpha: contactAlpha,
        rotation: shotMove.rotation * (0.10 + approachT * 0.45),
        originX: 0.64,
        originY: 0.86
      });
    }
    if (followT > 0.01) {
      this.drawSingleAngleSprite(ctx, followSprite, {
        x: 814 + shotMove.translateX * 0.45,
        y: 316 + shotMove.translateY * 0.20,
        width: 132,
        height: 222,
        alpha: followT,
        rotation: shotMove.rotation * 0.70,
        scaleX: shotMove.followScaleX,
        scaleY: shotMove.followScaleY,
        originX: 0.62,
        originY: 0.88
      });
    }
