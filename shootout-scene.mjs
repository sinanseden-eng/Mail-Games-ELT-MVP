...
import {
  ...
  mapCameraPoint,
  penaltyCameraFrame
} from "./shootout-cinematics.mjs?v=0.9.23";

import {
  PenaltyVisualPack,
  REALISTIC_SELECTION_POINTS,
  penaltyActionImpactPointForViewer
} from "./penalty-visuals.mjs?v=0.9.23";
import {
  PENALTY_VIEWERS,
  ...
  perspectiveLabel
} from "./penalty-perspective.mjs?v=0.9.23";
...
export class ShootoutScene {
  ...
  layoutZoneButtons() {
    for (const zone of ZONES) {
      ...
      if (!button) continue;
      const point = REALISTIC_SELECTION_POINTS.zones[zone.id];
      if (!point) continue;
