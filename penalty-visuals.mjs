...
export const PENALTY_VISUAL_ASSETS = Object.freeze({
  ...
  singleAngle: Object.freeze({
    ...
    keeperHighLeft: "./assets/penalty-single-angle/keeper-high-right.png",
    keeperHighRight: "./assets/penalty-single-angle/keeper-high-left.png",
    keeperLowLeft: "./assets/penalty-single-angle/keeper-low-right.png",
    keeperLowRight: "./assets/penalty-single-angle/keeper-low-left.png"
  })
});
...
export function penaltyFlightPoint(replay = {}, rawT = 0) {
  ...
  const start = REALISTIC_SELECTION_POINTS.ball;
...
export const SINGLE_ANGLE_KEEPER_MOVES = Object.freeze({
  "top-left": Object.freeze({ asset: "keeperHighLeft", x: 250, y: 292, width: 350, height: 170, rotation: -0.045, label: "HIGH LEFT" }),
  "top-centre": Object.freeze({ asset: "keeperReady", x: 570, y: 264, width: 140, height: 190, rotation: 0, label: "HIGH CENTRE" }),
  "top-right": Object.freeze({ asset: "keeperHighRight", x: 680, y: 292, width: 350, height: 170, rotation: 0.045, label: "HIGH RIGHT" }),
  "bottom-left": Object.freeze({ asset: "keeperLowLeft", x: 270, y: 378, width: 320, height: 147, rotation: -0.025, label: "LOW LEFT" }),
  "bottom-centre": Object.freeze({ asset: "keeperReady", x: 565, y: 366, width: 150, height: 150, rotation: 0, label: "LOW CENTRE", scaleX: 1.12, scaleY: 0.84 }),
  "bottom-right": Object.freeze({ asset: "keeperLowRight", x: 690, y: 378, width: 320, height: 147, rotation: 0.025, label: "LOW RIGHT" })
});
...
export function singleAngleBallState(replay = {}, progress = 0) {
  ...
  const target = outcome === "miss"
    ? penaltyMissPoint(zoneId, replay)
    : { ...REALISTIC_SELECTION_POINTS.zones[zoneId], label: zoneLabel(zoneId) };
  const rawFlightT = clamp((p - strike) / Math.max(0.001, contact - strike), 0, 1);
  const flightT = naturalBallFlightEasing(rawFlightT);
  const control = {
  ...
  let x = u*u*start.x + 2*u*flightT*control.x + flightT*flightT*target.x;
  let y = u*u*start.y + 2*u*flightT*control.y + flightT*flightT*target.y;
  let radius = lerp(21.5, 9.4, Math.pow(flightT, 0.84));
  ...
  if (p >= contact) {
    if (outcome === "goal") {
      ...
      radius = lerp(9.4, 7.8, settleT);
      rotation += settleT * Math.PI * 6 * side;
    }
  }
  const shadowY = lerp(651, zoneId.startsWith("top") ? 512 : 525, flightT);
  return {
    progress: p, outcome, strike, contact, zoneId, move, start, target, control,
    rawFlightT, flightT, settleT, x, y, radius, rotation,
    visible: p >= strike - 0.004,
    shadow: { x: lerp(start.x, target.x, flightT), y: shadowY, radiusX: lerp(22, 7, flightT), radiusY: lerp(6, 2.4, flightT), opacity: clamp(0.24 - Math.max(0, shadowY - y) / 900, 0.035, 0.21) }
  };
}
...
export class PenaltyVisualPack {
  ...
  ready() {
    return Boolean(this.get(this.assets.striker[0]));
  }

  drawSelection(ctx, { role = "striker", preview = null, active = true, time = 0 } = {}) {
    if (!this.enabled) return false;
    const image = this.get(this.assets.striker[0]);
    if (!image) {
      this.drawLoadingFrame(ctx);
      return false;
    }

    const breathe = Math.sin(time * 0.00115) * 0.0022;
    this.drawFullFrame(ctx, image, 1, {
      zoom: 1.008 + breathe,
      panX: Math.sin(time * 0.00023) * 1.8,
      panY: Math.cos(time * 0.00019) * 1.1,
      label: role === "keeper" ? "GOALKEEPER VIEW" : "STRIKER VIEW"
    });

    // Keep the goal mouth legible below the interactive buttons.
    ctx.save();
    const goalGlow = ctx.createRadialGradient(640, 278, 80, 640, 278, 530);
  ...
  drawSelectionGuide(ctx, role, zoneId, active, time) {
    const target = REALISTIC_SELECTION_POINTS.zones[zoneId] || REALISTIC_SELECTION_POINTS.zones["bottom-centre"];
    const start = role === "keeper" ? REALISTIC_SELECTION_POINTS.keeper : REALISTIC_SELECTION_POINTS.ball;
  ...
  drawSingleAngleCinematic(ctx, replay, time = 0, reducedMotion = false, final = false) {
    ...
    this.drawFullFrame(ctx, background, 1, { zoom: 1, panX: 0, panY: 0, label: "PENALTY REPLAY · MAIN CAMERA" });

    const keeperState = singleAngleKeeperState(replay, progress);
    this.drawSingleAngleKeeper(ctx, keeperState, time, reducedMotion);

    const followT = final ? 1 : smoothstep(clamp((progress - strike + 0.002) / 0.115, 0, 1));
    const contactAlpha = 1 - followT;
    if (contactAlpha > 0.01) {
      this.drawSingleAngleSprite(ctx, contactSprite, {
        x: 314 + shotMove.translateX * 0.35, y: 25 + shotMove.translateY * 0.25, width: 587, height: 662,
        alpha: contactAlpha, rotation: shotMove.rotation * 0.45, originX: 0.64, originY: 0.86
      });
    }
    this.drawSingleAngleSprite(ctx, followSprite, {
      x: 368 + shotMove.translateX, y: 25 + shotMove.translateY, width: 394, height: 664,
      alpha: Math.max(0.02, followT), rotation: shotMove.rotation, scaleX: shotMove.followScaleX, scaleY: shotMove.followScaleY, originX: 0.62, originY: 0.88
    });
    ...
    if (ball.visible) {
      ...
      if (!reducedMotion && progress < ball.contact && ball.rawFlightT > 0.10) {
        const ghost = singleAngleBallState(replay, Math.max(strike, progress - 0.018));
        ctx.save();
        ctx.globalAlpha = clamp((1 - ball.rawFlightT) * 0.10 + 0.035, 0.035, 0.12);
        this.drawBallMarker(ctx, ghost.x, ghost.y, ghost.radius * 0.88, ghost.rotation, { glow: false });
        ctx.restore();
      }
    ...
    if (replay.outcome === "goal" && progress >= ball.contact) {
      this.drawIntegratedNetRipple(ctx, ball.target, penaltyZoneEffect(shotZone), ball.settleT, time, final);
    }
  ...
  drawSingleAngleKeeper(ctx, state, time = 0, reducedMotion = false) {
    ...
    const start = { x: 585, y: 330, width: 110, height: 150 };
    ...
    const shadowY = state.zoneId.startsWith("top") ? 500 : 515;
    ctx.save();
    ctx.globalAlpha = clamp(0.24 - t * 0.08, 0.10, 0.24);
    ctx.fillStyle = "rgba(0,0,0,.74)";
    ctx.filter = "blur(4px)";
    ctx.beginPath();
    ctx.ellipse(shadowX, shadowY, lerp(34, width * 0.28, t), lerp(7, 4, t), 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.filter = "none";
    ctx.restore();
  ...
  drawSingleAngleSprite(ctx, image, { x, y, width, height, alpha = 1, rotation = 0, scaleX = 1, scaleY = 1, originX = 0.5, originY = 0.5 } = {}) {
    ...
    ctx.drawImage(image, -width * originX, -height * originY, width, height);
    ctx.restore();
  }
  ...
  drawKeeperNaturalReplay(ctx, replay, progress, time = 0, reducedMotion = false, final = false) {
    ...
    if (state.outcome === "goal" && state.progress >= state.contact) {
      this.drawIntegratedNetRipple(ctx, state.target, state.effect, state.settleT, time, final);
    }
  ...
  drawIntegratedNetRipple(ctx, point, effect, rawT = 0, time = 0, final = false) {
    const t = clamp(rawT, 0, 1);
    const pulse = final ? 0.16 : Math.sin(Math.min(1, t * 1.35) * Math.PI) * Math.exp(-t * 1.8);
    if (pulse <= 0.012) return;
    const side = effect.side || 1;
    const high = effect.level === "high";
    const spanX = high ? 62 : 76;
    const spanY = high ? 52 : 38;
    const push = (high ? 18 : 14) * pulse;
    const oscillation = Math.sin(time * 0.018 + t * Math.PI * 5) * 3 * pulse;

    ctx.save();
    ctx.globalAlpha = clamp(0.07 + pulse * 0.20, 0, 0.24);
    ctx.strokeStyle = "rgba(226,231,226,.76)";
    ctx.lineWidth = 0.9;
    ctx.lineCap = "round";

    for (let index = -2; index <= 2; index += 1) {
      const x = point.x + index * (spanX / 2.4);
      ctx.beginPath();
      ctx.moveTo(x, point.y - spanY);
      ctx.quadraticCurveTo(
        x + side * push * (1 - Math.abs(index) / 3),
        point.y + push + oscillation,
        x + side * pulse * 4,
        point.y + spanY
      );
      ctx.stroke();
    ...
    for (let index = -2; index <= 2; index += 1) {
      const y = point.y + index * (spanY / 2.4);
      ctx.beginPath();
      ctx.moveTo(point.x - spanX, y);
      ctx.quadraticCurveTo(
        point.x + side * push + oscillation,
        y + push * 0.56 * (1 - Math.abs(index) / 3),
        point.x + spanX,
        y + pulse * 3
      );
      ctx.stroke();
