...
const matchId = "63333333-3333-4333-8333-333333333333";
const turnBId = "64444444-4444-4444-8444-444444444444";
const turnAId = "65555555-5555-4555-8555-555555555555";
const match = {
  id: matchId,
  game_type: "turkey",
  status: "active",
  ...
  question_pack_id: "61111111-1111-4111-8111-111111111111",
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (target === "https://gmail.googleapis.com/gmail/v1/users/me/messages/send") {
    gmailCounter += 1;
    return response({ id: `gmail-turkey-${gmailCounter}`, threadId: `thread-turkey-${gmailCounter}` });
  }
...
test("0.8 Fighter B submission emails Player A a fight replay and Player A the next turn", async () => {
  requests.length = 0;
  gmailCounter = 0;
  ...
  const result = await handler({
    httpMethod: "POST",
    body: JSON.stringify({ token, answer: "had", move: "block" })
  });

  assert.equal(result.statusCode, 200);
  const payload = JSON.parse(result.body);
  assert.equal(payload.outcome.resolved, true);
  assert.equal(payload.outcome.replay.gameType, "turkey");
  ...
  assert.equal(payload.resultEmail.gameType, "turkey");
  ...
  assert.ok(messages.some(message => /To: a@example\.com/.test(message) && /Watch the fight/.test(message)));
